...
export function DeepseekAssistant({ onContentGenerate, selectedText }: DeepseekAssistantProps) {
  const [prompt, setPrompt] = useState('');
  const [tone, setTone] = useState('professional');
  const [model, setModel] = useState('deepseek-r1-chat');
  ...
  const generateContent = async () => {
    ...
    try {
      const content = await generateWithDeepseek(
        `Generate newsletter content about: ${prompt}\nTone: ${tone}`,
        { model }
      );
