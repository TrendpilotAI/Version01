...
DO $$ BEGIN
  DROP POLICY IF EXISTS workspace_feed_settings_access ON feed_settings;
  DROP POLICY IF EXISTS workspace_feed_items_access ON feed_items;
END $$;

-- RLS Policies
CREATE POLICY "feed_settings_workspace_access"
  ON feed_settings
  FOR ALL
  USING (
    EXISTS (
...
CREATE POLICY "feed_items_workspace_access"
  ON feed_items
  FOR ALL
  USING (
    EXISTS (
